...
from NSA.train_mvtec import prepare_nsa
import random
from NSA.self_sup_data.self_sup_tasks import patch_ex
class MVTecTestDataset(Dataset):
    ...
    def transform_image(self, image_path, mask_path):
        ...
        gx = cv2.Sobel(imagegray, cv2.CV_64F, 1, 0, ksize=3)
        gy = cv2.Sobel(imagegray, cv2.CV_64F, 0, 1, ksize=3)
        # auggray=auggray[:,:,None]
        gx = gx[:, :, None].astype(np.float32)
        gy = gy[:, :, None].astype(np.float32)
        imagegray = np.concatenate([gx,gy],axis=2)
...
class MVTecTrainDataset(Dataset):
    ...
    def __init__(self, root_dir, anomaly_source_path, resize_shape=None):
        ...
        #introduce NSA aug
        class_name = self.root_dir.split('/')[-4]
        nsa_dir = root_dir.split(class_name)[0]
        self.NSA = prepare_nsa(nsa_dir,class_name)
    ...
    def augment_NSA(self, image):
        ...
        if no_anomaly > 0.5:
            ...
            if self.NSA.self_sup:
                p = random.choice(self.NSA.x)
                if self.NSA.transform is not None:
                    p = self.NSA.transform(p)
                p = np.asarray(p)
                augmented_image, msk = patch_ex(image, p, **self.NSA.self_sup_args)
            has_anomaly = 1.0
            if np.sum(msk) == 0:
                has_anomaly=0.0
    ...
    def transform_image(self, image_path, anomaly_source_path):
        ...
        if what_anomaly > 0.8:
            ...
            augmented_image, anomaly_mask, has_anomaly = self.augment_NSA(image)
            augmented_image, anomaly_mask, has_anomaly = self.augment_image(augmented_image, anomaly_source_path)
        #'''

        auggray = cv2.cvtColor(augmented_image, cv2.COLOR_BGR2GRAY)
        gx = cv2.Sobel(auggray, cv2.CV_64F, 1, 0, ksize=3)
        gy = cv2.Sobel(auggray, cv2.CV_64F, 0, 1, ksize=3)
        gx=gx[:,:,None].astype(np.float32)
        gy=gy[:,:,None].astype(np.float32)
        auggray = np.concatenate([gx,gy],axis=2)
